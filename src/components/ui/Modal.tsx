...
export function Modal({
  open, onOpenChange, title, description, children, footer, wide = false, dismissable = true,
}: {
  open: boolean
  ...
  footer?: ReactNode
  wide?: boolean
  /* false = a required decision: no close button, and Escape / clicking outside do nothing */
  dismissable?: boolean
}) {
  const block = (e: Event) => { if (!dismissable) e.preventDefault() }
  return (
    <Dialog.Root open={open} onOpenChange={(o) => { if (!o && !dismissable) return; onOpenChange(o) }}>
      <Dialog.Portal>
